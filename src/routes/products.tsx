...
function ProductsPage() {
  ...
  const saveProduct = async (product: Product) => {
    try {
      ...
      console.error(err);
      setError(
        getErrorMessage(err, "ပစ္စည်းများကို database တွင် သိမ်းဆည်း၍မရပါ"),
      );
      throw err;
    }
  };
  ...
  const add = async () => {
    ...
    const p: Product = { ...draft, id: crypto.randomUUID() };
    try {
      await saveProduct(p);
      setDraft({
        id: "",
      ...
      // The visible error banner already contains the failure reason.
    }
  };
  ...
  const del = async (id: string) => {
    if (
      !window.confirm(
        "ဒီပစ္စည်းကို ဖျက်မှာ သေချာပါသလား? အရောင်းမှတ်တမ်းရှိပြီးသား ပစ္စည်းကို ဖျက်၍မရပါ။",
      )
    )
      return;
    try {
      ...
      setError(getErrorMessage(err, "ပစ္စည်းဖျက်၍မရပါ"));
    }
  };
  ...
  const changeStock = async (id: string, delta: number) => {
    try {
      setError(null);
      await store.adjustProductStock(id, delta, "Quick stock adjustment");
      setProducts(await store.getProducts());
    } catch (err) {
      setError(getErrorMessage(err, "Stock ပြင်ဆင်၍မရပါ"));
    }
  };
  ...
  const saveEdit = async () => {
    if (!editDraft) return;
    const previous = products.find((product) => product.id === editDraft.id);
    const stockDelta = (editDraft.stock ?? 0) - (previous?.stock ?? 0);
    const updatedWithoutStock = { ...editDraft, stock: previous?.stock ?? 0 };
    try {
      await saveProduct(updatedWithoutStock);
      if (stockDelta !== 0) await changeStock(editDraft.id, stockDelta);
      setEditingId(null);
      setEditDraft(null);
    } catch {
      // saveProduct/changeStock already shows the error to the user.
    }
  };
    ...
    <Shell>
        ...
        <div className="mb-5 flex items-center gap-3 rounded-2xl border border-destructive/20 bg-destructive-soft/10 p-4 text-xs text-destructive">
          ...
          <div>
            <p className="font-semibold">
              စနစ်အမှား ဖြစ်ပွားပါသည် (Database Error):
            </p>
      ...
      <div className="mb-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
        ...
        >
          ...
          <div className="text-[11px] text-muted-foreground">
            စုစုပေါင်း {stats.totalUnits} ခု ကျန်
          </div>
        </button>
        <button
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {visibleProducts.map((p) => {
          ...
          if (isEditing && editDraft && canManageProducts) {
            return (
              <div
                key={p.id}
              ...
              <div className="flex items-center justify-between rounded-xl bg-secondary/60 px-3 py-2">
                ...
                {canManageProducts ? (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="w-14 rounded-md border border-border bg-background px-2 py-1 text-center text-sm font-semibold">
                      {p.stock ?? 0}
                    </span>
                    <button
                      onClick={() => changeStock(p.id, 1)}
                  ...
                  <span className="text-sm font-semibold">{p.stock ?? 0}</span>
                )}
              </div>
            </div>
          );
